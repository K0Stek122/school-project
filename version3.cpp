...
/*
  This version includes everything that is needed in the project,
  from this point on, all the changes will be strictly made to make the
  app user friendly, I have made a list of things to add to the project.
  - GUI or UI, i consider using either ncurses library to make a terminal GUI, or to use a library that was recommended by youtube channel: "One Lone Coder", the library he explained there is compatible with MacOS, Windows and Linux, which means the application will be backwards compatible with everything, NOTE: The library is called wxWidgets.
  - Dynamics, one of the most important things to add, this means that the UI will be responsive, if ncurses will be used, i do not know yet how to implement this.
  - When i was looking at similiar projects, i have noticed that they had explanations of how to convert all those numbers, i would like to do this as well.
 */
